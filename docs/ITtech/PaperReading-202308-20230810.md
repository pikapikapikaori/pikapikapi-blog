...
这里考虑的是对估值的分布$\bold{D}$仅知道其有关的部份信息。为了达到近似最佳，首先可以检察是否存在最大可行集$S\in\mathcal{S}$使得其期望值至少为$\frac{OPT}{\log\log k}$，如果存在那么可以通过接受该集合中的所有投标人（价格0）来达到近似最佳的目的。相反若不存在，那么可以使用一个任意小的步长$\delta>0$来统一提高每个活跃投标人的价格，使得他们有机会在每次价格上涨后退出。简单起见可以为所有投标人同时提供该价格，但在实践中以任意的顺序向每个投标人提供增加后的价格。同样当剩下的活跃投标人可行或其子集可行并达到预期的近似最优时拍卖结束。
